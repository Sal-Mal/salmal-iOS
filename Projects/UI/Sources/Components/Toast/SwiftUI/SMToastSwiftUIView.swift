...
public struct SMSwiftUIToast: Equatable {
  let type: SMToastSwiftUIView.ToastType
  let duration: TimeInterval

  public init(
    type: SMToastSwiftUIView.ToastType,
...
public struct SMToastSwiftUIView: View {
...
struct SMToastView_Previews: PreviewProvider {
  ...
  static var previews: some View {
    Group {
      SMToastSwiftUIView(for: .success("살말 업로드 완료!"))
        .previewDisplayName("성공 토스트")
        .previewLayout(.sizeThatFits)

      SMToastSwiftUIView(for: .warning("화면 캡처를 감지했어요."))
        .previewDisplayName("경고 토스트")
        .previewLayout(.sizeThatFits)

      SMToastSwiftUIView(for: .error("더 이상 해당 사용자가 피드에서 보이지 않아요."))
